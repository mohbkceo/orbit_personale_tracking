...
import { createTransaction } from '../services/financeService.js';
...
function crudRoutes(Model, schema, sort) {
  const router = Router();
  router.get('/', asyncHandler(async (_req, res) => success(res, await Model.find().populate('accountId').sort(sort))));
  router.post('/', validate(schema), asyncHandler(async (req, res) => success(res, await Model.create(req.body), 201)));
  router.patch('/:id', validate(schema.partial()), asyncHandler(async (req, res) => { const item = await Model.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true }); if (!item) throw new AppError('Record not found', 404); return success(res, item); }));
  router.delete('/:id', asyncHandler(async (req, res) => { const item = await Model.findByIdAndDelete(req.params.id); if (!item) throw new AppError('Record not found', 404); return success(res, { deleted: true }); }));
  return router;
}
...
billRoutes.post('/:id/pay', validate(z.object({ accountId: id.optional(), date: z.coerce.date().optional() })), asyncHandler(async (req, res) => {
  ...
  if (bill.autoCreateExpense) await createTransaction({ type: 'expense', amount: bill.amount, accountId, category: bill.category, description: bill.name, date: req.body.date || new Date(), sourceEntityType: 'Bill', sourceEntityId: bill._id, createdVia: 'web' });
...
goalRoutes.post('/:id/contribute', validate(z.object({ amount: z.coerce.number().positive(), accountId: id, destinationAccountId: id })), asyncHandler(async (req, res) => {
  const goal = await Goal.findById(req.params.id); if (!goal) throw new AppError('Goal not found', 404);
  await createTransaction({ type: 'savings_contribution', amount: req.body.amount, accountId: req.body.accountId, destinationAccountId: req.body.destinationAccountId, category: 'Savings', description: `Contribution to ${goal.title}`, sourceEntityType: 'Goal', sourceEntityId: goal._id, createdVia: 'web' });
...
goalRoutes.post('/:id/withdraw', validate(z.object({ amount: z.coerce.number().positive(), accountId: id, destinationAccountId: id })), asyncHandler(async (req, res) => {
  ...
  await createTransaction({ type: 'savings_withdrawal', amount: req.body.amount, accountId: req.body.accountId, destinationAccountId: req.body.destinationAccountId, category: 'Savings', description: `Withdrawal from ${goal.title}`, sourceEntityType: 'Goal', sourceEntityId: goal._id, createdVia: 'web' });
