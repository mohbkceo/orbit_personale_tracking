...
accountRoutes.get('/', asyncHandler(async (req, res) => success(res, await accountBalances(req.user._id, req.query.includeArchived === 'true' ? {} : { archived: false }))));
...
accountRoutes.post('/', validate(accountInput), asyncHandler(async (req, res) => {
  ...
  await recordActivity(req.user._id, { action: 'created', entityType: 'Account', entityId: account._id, description: `Created ${account.name}`, newData: account.toObject() });
...
accountRoutes.delete('/:id', asyncHandler(async (req, res) => {
  const [account, linked] = await Promise.all([Account.findOne({ _id: req.params.id, user: req.user._id }), Transaction.exists({ user: req.user._id, $or: [{ accountId: req.params.id }, { destinationAccountId: req.params.id }], deletedAt: null })]);
  if (!account) throw new AppError('Account not found', 404);
...
accountRoutes.post('/:id/archive', asyncHandler(async (req, res) => {
  const account = await Account.findOneAndUpdate({ _id: req.params.id, user: req.user._id }, { archived: true }, { new: true });
  if (!account) throw new AppError('Account not found', 404);
  await recordActivity(req.user._id, { action: 'archived', entityType: 'Account', entityId: account._id, description: `Archived ${account.name}` });
  return success(res, account);
}));
