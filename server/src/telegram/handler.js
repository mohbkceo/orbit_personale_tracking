...
const money = (amount, currency = 'DZD') =>
  new Intl.NumberFormat('en-DZ', { style: 'currency', currency, maximumFractionDigits: 0 }).format(
    amount,
  );
...
async function executeIntent(parsed, settings) {
  ...
  if (['SHOW_TODAY', 'SHOW_MONEY', 'SHOW_DEBTS'].includes(parsed.intent))
    return { text: await summaries(parsed.intent) };
  if (parsed.intent === 'CREATE_EXPENSE' || parsed.intent === 'CREATE_INCOME') {
    ...
    return {
      text: `✓ ${isExpense ? 'Expense' : 'Income'} added\n\n<b>${money(item.amount, settings.defaultCurrency)}</b>\n${item.description}\n${item.category}`,
      markup: keyboard(['Undo', `undo:transaction:${item._id}`]),
    };
  }
  if (parsed.intent === 'CREATE_TASK') {
    const item = await createTask({ ...parsed.data, createdVia: 'telegram' });
    return {
      text: `✓ Task created\n\n<b>${item.title}</b>${item.dueDate ? `\n${dayjs(item.dueDate).format('ddd, D MMM')}` : ''}`,
      markup: keyboard(['Done', `done:task:${item._id}`], ['Delete', `undo:task:${item._id}`]),
    };
  }
  if (parsed.intent === 'CREATE_RECEIVABLE' || parsed.intent === 'CREATE_PAYABLE') {
    ...
    return {
      text: `✓ ${type === 'receivable' ? 'Receivable' : 'Payable'} created\n\n<b>${item.personName}</b>\n${money(item.originalAmount, item.currency)}`,
    };
  }
  if (parsed.intent === 'RECORD_RECEIVABLE_PAYMENT' || parsed.intent === 'RECORD_PAYABLE_PAYMENT') {
    ...
    const debt = await Debt.findOne({
      type,
      personName: new RegExp(
        `^${parsed.data.personName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`,
        'i',
      ),
      status: { $ne: 'paid' },
      archived: false,
    ...
    const updated = await recordDebtPayment(debt._id, {
  ...
  return {
    text: 'I did not understand that yet. Try “spent 500 coffee”, “income 15000 freelance”, or “task call dentist tomorrow”.',
  };
}
...
async function handleCallback(callback, settings, token) {
  ...
  await telegramRequest(token, 'answerCallbackQuery', {
    ...
    text: action === 'done' ? 'Task completed' : 'Action undone',
  });
...
export async function handleTelegramUpdate(update) {
  ...
  if (text === '/tasks') {
    ...
    return sendMessage(token, chatId, `<b>Tasks</b>\n\n${count} open`);
  }
  if (text === '/accounts') {
    const accounts = await accountBalances({ archived: false });
    return sendMessage(
      token,
      chatId,
  ...
  if (text === '/goals') {
    const rows = await Goal.find({ status: 'active' }).sort({ targetDate: 1 }).limit(8);
    return sendMessage(
      token,
      chatId,
  ...
  if (text === '/bills') {
    const rows = await Bill.find({ status: { $ne: 'paid' } })
      .sort({ dueDate: 1 })
      .limit(8);
    return sendMessage(
      token,
  ...
  if (text === '/subscriptions') {
    const rows = await Subscription.find({ status: 'active' })
  ...
  if (['/expenses', '/income'].includes(text)) {
    const type = text.slice(1);
    const rows = await Transaction.find({ type, deletedAt: null }).sort({ date: -1 }).limit(5);
    return sendMessage(
      token,
      chatId,
