...
export async function dashboardSummary(userId) {
  ...
  const [accounts, totals, debtTotals, taskCounts, recentTransactions, upcomingBills, upcomingSubscriptions, goals, activity] = await Promise.all([
    ...
    Task.aggregate([{ $match: { user, archived: false } }, { $group: { _id: '$status', total: { $sum: 1 }, overdue: { $sum: { $cond: [{ $and: [{ $lt: ['$dueDate', new Date()] }, { $not: { $in: ['$status', ['completed', 'cancelled']] } }] }, 1, 0] } } } }]),
  ...
  return {
    ...
    tasks: { open: openTasks, overdue, dueToday: await Task.countDocuments({ user, archived: false, status: { $in: ['todo', 'in_progress'] }, dueDate: { $gte: dayjs().startOf('day').toDate(), $lte: todayEnd } }) },
