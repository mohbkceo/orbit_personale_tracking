...
export async function ensureAccount(id, label = 'Account') {
  if (!mongoose.isValidObjectId(id)) throw new AppError(`${label} is invalid`, 400);
  const account = await Account.findOne({ _id: id, archived: false });
  if (!account) throw new AppError(`${label} was not found`, 404);
  return account;
}

export async function createTransaction(input) {
  const account = await ensureAccount(input.accountId);
  let destination;
  if (destinationTypes.includes(input.type)) {
    ...
    destination = await ensureAccount(input.destinationAccountId, 'Destination account');
  ...
  return transaction.populate(['accountId', 'destinationAccountId']);
}
...
export async function updateTransaction(id, input) {
  ...
  await ensureAccount(merged.accountId);
  if (destinationTypes.includes(merged.type)) await ensureAccount(merged.destinationAccountId, 'Destination account');
  Object.assign(current, input);
  await current.save();
  await recordActivity({ action: 'updated', entityType: 'Transaction', entityId: current._id, description: `Updated ${current.description}`, previousData: input, newData: current.toObject() });
  return current.populate(['accountId', 'destinationAccountId']);
}

export async function archiveTransaction(id, source = 'web') {
  const transaction = await Transaction.findOneAndUpdate({ _id: id, deletedAt: null }, { deletedAt: new Date() }, { new: true });
  if (!transaction) throw new AppError('Transaction not found', 404);
  await recordActivity({ action: 'archived', entityType: 'Transaction', entityId: transaction._id, description: `Archived ${transaction.description}`, source });
  return transaction;
}
...
export async function listTransactions(query = {}) {
  ...
  const [data, total] = await Promise.all([
    Transaction.find(filter).populate('accountId destinationAccountId').sort({ date: -1, createdAt: -1 }).skip(skip).limit(limit),
    Transaction.countDocuments(filter),
  ]);
  return { data, pagination: { page, limit, total, pages: Math.ceil(total / limit) } };
}

export async function accountBalances(accountFilter = {}) {
  const accounts = await Account.find(accountFilter).sort({ archived: 1, createdAt: 1 }).lean();
